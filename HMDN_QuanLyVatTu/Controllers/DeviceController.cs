...
namespace HMDN.Controllers.API
{
    ...
    public class DeviceController : ApiController
    {
        ...
        // Đổi thành URL domain thật của bệnh viện nha bro
        private const string BASE_URL = "http://hoshi/";

        [HttpGet]
        public IHttpActionResult List()
        {
            var data = db.Database
                .SqlQuery<InventoryListVM>("EXEC sp_GetInventoryList")
                .ToList();
            return Ok(data);
        }
        ...
        {
            db.Database.ExecuteSqlCommand(
                "EXEC sp_Device_Suspend @Id, @Reason",
                new SqlParameter("@Id", model.Id),
                new SqlParameter("@Reason", model.Reason)
            );
        ...
        {
            db.Database.ExecuteSqlCommand(
                "EXEC sp_Device_ChangeStatus @Id, @Status",
                new SqlParameter("@Id", model.Id),
                new SqlParameter("@Status", model.Status)
            );
            return Ok();
        ...
        public IHttpActionResult Create(CreateInventoryVM model)
        {
            var result = db.Database
                .SqlQuery<CreateInventoryResult>(
                    ...
                    new SqlParameter("@CheckCycleId", (object)model.CheckCycleId ?? DBNull.Value),
                    new SqlParameter("@UnitPrice", model.UnitPrice),
                    ...
                    new SqlParameter("@ResidualValue", (object)model.ResidualValue ?? DBNull.Value),
                    new SqlParameter("@ApprovedQuantity", (object)model.ApprovedQuantity ?? DBNull.Value),
                    ...
                    new SqlParameter("@UsageYears", (object)model.UsageYears ?? DBNull.Value),
                    new SqlParameter("@AssetCategory", (object)model.AssetCategory ?? DBNull.Value),
                    new SqlParameter("@GroupAssetCode", (object)model.GroupAssetCode ?? DBNull.Value),
                    new SqlParameter("@AccountingCode", (object)model.AccountingCode ?? DBNull.Value),
                    new SqlParameter("@InsuranceCode", (object)model.InsuranceCode ?? DBNull.Value),
                    ...
                    new SqlParameter("@BaseUrl", BASE_URL)
                )
                .FirstOrDefault();
        ...
        {
            db.Database.ExecuteSqlCommand(
                @"EXEC sp_Device_Update
                    @Id, @AssetCode, @ItemId, @SerialNumber, @Quantity,
                    @DepartmentId, @LocationId,
                    @ImportDate, @ExpiryDate, @WarrantyExpiry,
                    @CheckCycleId, @UnitPrice,
                    @DepreciationRate, @DepreciationYears, @ResidualValue,
                ...
                new SqlParameter("@Manufacturer", (object)model.Manufacturer ?? DBNull.Value),
                new SqlParameter("@SupplierName", (object)model.SupplierName ?? DBNull.Value),
                new SqlParameter("@QrCode", DBNull.Value),
                new SqlParameter("@Note", (object)model.Note ?? DBNull.Value),
                new SqlParameter("@IdTicket", (object)model.IdTicket ?? DBNull.Value)
            );
            return Ok();
        ...
        {
            var data = db.Database
                .SqlQuery<MaintenanceLog>(
                    "EXEC sp_Device_GetHistory @InventoryId",
        ...
        {
            var data = db.Database.SqlQuery<DropdownVM>(
                "SELECT Id, Name FROM Departments WHERE IsActive = 1 ORDER BY Name"
            ).ToList();
            return Ok(data);
        }
        ...
        {
            var data = db.Database.SqlQuery<DropdownVM>(
                "SELECT Id, Name FROM Locations WHERE IsActive = 1 ORDER BY Name"
            ).ToList();
            return Ok(data);
        }
        ...
        {
            var data = db.Database.SqlQuery<TicketDropdownVM>(
                "SELECT Id, TicketCode FROM Tickets ORDER BY TicketCode"
            ).ToList();
            return Ok(data);
        }
        ...
        {
            var data = db.Database.SqlQuery<DropdownVM>(
                "SELECT Id, Name + ' (' + CycleType + ')' AS Name FROM CheckCycles"
            ).ToList();
            return Ok(data);
        }
