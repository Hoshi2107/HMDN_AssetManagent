...
-- Drop existing sp_RunAlertDiagnostics
...
-- Create updated sp_RunAlertDiagnostics with partition-by-device aggregation and auto-dismissal logic
...
BEGIN
    ...
    -- ── RULE 4: CHECKLIST OVERDUE (WITH AGGREGATION) ──
    ...
    BEGIN
        ...
        -- Auto-resolve older active alerts for these devices if a different due date is being alerted
        UPDATE a
        SET a.IsResolved = 1,
            a.ResolvedAt = GETDATE(),
            a.ResolvedBy = 1
        ...
        WHERE a.AlertRuleId = @ChecklistRuleId
          AND a.IsResolved = 0
          AND a.Body NOT LIKE '%' + CONVERT(NVARCHAR(10), lo.DueDate, 103) + '%';

        -- Aggregate by Group and CycleType
        SELECT 
            g.Id AS GroupId,
            g.Name AS GroupName,
            lo.CycleType,
        ...
        JOIN Inventory inv ON lo.InventoryId = inv.Id
        JOIN Items it ON inv.ItemId = it.Id
        LEFT JOIN Groups g ON it.GroupId = g.Id
        GROUP BY g.Id, g.Name, lo.CycleType;
        ...
        SELECT
            @ChecklistRuleId,
            ao.RepInventoryId,
            N'Quá hạn checklist bảo trì (' + CAST(ao.OverdueCount AS NVARCHAR(10)) + N' thiết bị)',
            N'Có ' + CAST(ao.OverdueCount AS NVARCHAR(10)) + N' thiết bị ' + ISNULL(ao.GroupName, N'Vật tư') + N' (ví dụ mã: ' + inv.AssetCode + N') quá hạn checklist định kỳ (' + 
                CASE ao.CycleType 
                    WHEN 'monthly' THEN N'Hàng tháng'
                    WHEN 'yearly' THEN N'Hàng năm'
                    WHEN 'weekly' THEN N'Hàng tuần'
                    ELSE N'Hàng ngày' 
                END + N') từ ngày ' + CONVERT(NVARCHAR(10), ao.MinDueDate, 103) + N'. Yêu cầu bộ phận kỹ thuật thực hiện kiểm tra khẩn cấp.',
            'info',
            0,
            0,
            GETDATE()
    ...
    -- ── RULE 4B: CHECKLIST DUE (CYCLE-AWARE WARNINGS & AGGREGATION) ──
    ...
    BEGIN
        ...
        WHERE a.AlertRuleId = @ChecklistDueRuleId
          AND a.IsResolved = 0
          AND a.Body NOT LIKE '%' + CONVERT(NVARCHAR(10), ld.DueDate, 103) + '%';
        ...
        INTO #AggregatedDue
        FROM #LatestDue ld
        JOIN Inventory inv ON ld.InventoryId = inv.Id
        JOIN Items it ON inv.ItemId = it.Id
        LEFT JOIN Groups g ON it.GroupId = g.Id
        GROUP BY g.Id, g.Name, ld.CycleType;
        ...
        SELECT
            @ChecklistDueRuleId,
            ad.RepInventoryId,
            N'Lịch kiểm tra sắp đến hạn (' + CAST(ad.DueCount AS NVARCHAR(10)) + N' thiết bị)',
            N'Có ' + CAST(ad.DueCount AS NVARCHAR(10)) + N' thiết bị ' + ISNULL(ad.GroupName, N'Vật tư') + N' (ví dụ mã: ' + inv.AssetCode + N') sắp đến hạn kiểm tra/bảo trì định kỳ (' + 
                CASE ad.CycleType 
                    WHEN 'monthly' THEN N'Hàng tháng'
                    WHEN 'yearly' THEN N'Hàng năm'
                    WHEN 'weekly' THEN N'Hàng tuần'
...
PRINT 'sp_RunAlertDiagnostics updated successfully with checklist aggregation and auto-dismissal logic.';
